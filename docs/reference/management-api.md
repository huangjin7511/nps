...
如果只是部署单台服务端，优先看 [服务端运维](#/guide/server/operations)。
...
| 任务 | 页面 |
| --- | --- |
| 查接入入口和登录 | [管理接入入口](#/reference/integration/management-api-entrypoints) |
| 查 HTTP 路径 | [HTTP 接口目录](#/reference/management-api-http) |
| 查实时事件、WS、callback、webhook | [实时通道与回调](#/reference/management-api-realtime) |
