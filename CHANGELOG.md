...
### v0.35.0 (2026-03-31)
- 新增节点管理控制面，正式提供 `/api/*` 管理接口，支持总览快照、批量请求、全量配置导入导出与运行态同步
- 节点支持多平台接入与 `direct` / `reverse` / `dual` 连接模式，补齐 reverse WS、callback、失败队列补偿、重放与签名能力
- 管理页面与后端服务层围绕用户、客户端、隧道、域名和节点管理做了重构，调整页面结构并补充对应接口
- 优化 P2P 会话处理、遥测导出与运行诊断，增强复杂网络环境下的稳定性与可观测性
- 修复并清理一批测试、竞态、lint 与静态检查问题，补充相关单元测试覆盖
- 调整构建脚本、打包流程、Docker 相关配置并更新依赖
